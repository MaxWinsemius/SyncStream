...
class VirtualInterface(common.Interface):
    ...
    def get_child_at_index(self, index):
        ...
        raise VirtualInterfaceOutOfBoundsException(f"Tried to get child of framebuffer index {index} whilst Virtual Interface has length {self.length}.")

    def get_children_over_range(self, start, stop):
        """Finds the children that have the leds over the range of indices. Note that stop is exclusive, as if you are
    ...
    def append_interface(self, interface):
        """Appends the given interface to this virtual interface, also checks if the new interface has this (self) as part of its children either, to prevent loops."""
        self.insert_interface(interface, self.length)

    def insert_interface(self, interface, index=0):
        """Inserts the given interface tot his virtual interface, and also checks if the new interface has this (Self) as part of its childeren either, to prevent loops."""
         # TODO: check to prevent self-loops

        if(len(self.children) > index): # Insert in between two older interfaces
        ...
        else: # Index is bigger than amount of children - 1
    ...
    def remove_interface(self, index):
        ...
        if index < 0:
            raise ValueError("Trying to remove negative index from Virtual Interface")
    ...
    def send_udp_framebuffer(self):
        for child in self.children:
            ...
            # UNCOMMENTED since the framebuffer of the virtual interface is not set at this moment in time, since when
            # setting the color, it can directly be propagated to the children
            #child.set_color_array(child.start, child.stop, self.framebuffer[child.start:child.stop])
    ...
    def set_color_array(self, start, stop, color_array):
        ...
        for child in children:
            # ca_start = index in color_array where the current child starts reading from
            # child_start = start index in child framebuffer where subsection of color_array is mapped to
            # ca_stop = index in color_array where the current child stops reading from
            # child_stop = stop index in child framebuffer where the subsection from the color_array is mapped to
            ...
            child.set_color_array(child_start, child_stop, color_array[ca_start:ca_stop])
            #print(f"child.set_color_array({child_start}, {child_stop}, color_array[{ca_start}:{ca_stop}])")

    def set_rgb_array(self, start, stop, rgb):
        self.check_start_stop_range(start, stop, len(rgb) / 3)
        self.set_color_array(start, stop, [common.Clr(r, g, b) for r, g, b in [rgb[i:i+3] for i in range (0, len(rgb), 3)]]) # wow such beautiful much python code
    ...
    def set_updating_range(start, stop, update):
