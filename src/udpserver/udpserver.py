...
def parse_interface(config):
    ...
    if config["interface-type"] == "virtual":
        ...
        if len(config["children"]) < 1:
            raise ValueError(
                f"Virtual interface {config['name']} has no children")

        for child in config["children"]:
            inf.append_interface(parse_interface(child))

    elif config["interface-type"] == "physical":
        inf = physical.PhysicalInterface(
            config["ip"],
            config["port"],
            config["amount-leds"],
            config["max-udp-size"],
            config["max-brightness"])
...
def launch_interface(root):
    ...
    sovereign.Root(parse_interface(root),
                   config["sockets"]["dir"] + "/" + root["name"])
