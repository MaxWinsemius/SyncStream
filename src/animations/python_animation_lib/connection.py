...
class DummyConnection:
    """Dummy connection, reads YAML content from file instead of ICP socket."""
    def __init__(self, socket_path):
        self.socket = socket_path.split("/")[-1]
        print(f"Dummy connection to {socket_path} (config = {self.socket})!")

    def get_something(self, type, payload=""):
        with open("dummy/" + self.socket + ".yaml", "r") as f:
            data = f.read()
        return data

    def send_buffer(self, data):
        print(f"Sent {data} to {self.socket}.")

    def close(self):
        print(f"Socket {self.socket} closed!")
...
class Connection:
    ...
    def __init__(self, socket_path, auto_reconnect=True):
